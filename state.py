...
def default_columns():
    ...
    return [
        ...
        {"id": uid("col"), "type": "longtext", "label": "Catatan", "order": 6, "deletable": True,
...
def migrate(state: dict) -> dict:
    ...
    removed_col_ids = {c["id"] for c in state.get("columns", []) if c["type"] == "supplier-pick"}
    if removed_col_ids:
        state["columns"] = [c for c in state["columns"] if c["id"] not in removed_col_ids]
        for it in state.get("items", []):
...
def compute_summary(state):
    ...
    - supplier_recommended: HARGA YANG DICADANGKAN (RM) — per supplier, the
      total of only the items where they are the item's Kewangan (K) pick.
    - grand_total: JUMLAH KESELURUHAN PEROLEHAN (RM) — sum of supplier_recommended,
      i.e. the final total procurement value.
    - has_pick_column: whether a Kewangan (K) pick column exists, so callers can
      show a helpful note instead of a silently-zero total.
    """
    ...
    pick_col = next((c for c in state["columns"] if c.get("tag") == "kewangan_pick"), None)
